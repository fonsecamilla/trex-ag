...
def populacao_aleatoria(n):
    ...
    individuos = []
    for i in range(n):
       indv = np.random.uniform(-20,20,(3,10))
       individuos.append(indv)
    return individuos
    ...
    for i in range(individuo.shape[0]):
        for j in range(len(individuo[0])):
            ...
            if chance <= CHANCE_MUT:
                multiplicador = np.random.uniform(0.7, 1.8) #intervalo pequeno pra mut. nao ser mt drastica
                individuo[i][j] *= multiplicador
...
def proxima_geracao(populacao, fitness):
    ...
    pop_ord = ordenar_lista(populacao, fitness)
    pop_mantida = pop_ord[:NUM_MELHORES]
    for ind in pop_mantida:
        proxima_ger.append(ind)
    ...
    while len(proxima_ger) < NUM_INDIVIDUOS:
        ...
        mae, pai = random.choices(pop_mantida, k=2)
